...
var path = manifest.paths;
    path.source = 'src/assets/';
    path.dist = 'web/assets/';
    path.pages = 'web/pages/';
...
// - `globs.videos` - Array of video path globs.
...
var enabled = {
  ...
  // Add revisioned links for files in twig
  replaceFileLinks: argv.production
};
...
gulp.task('clean', require('del').bind(null, [path.dist, path.pages]));
...
gulp.task('compile', function () {
    'use strict';
    if (enabled.replaceFileLinks) {
      var manifest = JSON.parse(fs.readFileSync(revManifest, 'utf8'));
